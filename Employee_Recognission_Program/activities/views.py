...
def decline_activity_request(request,request_id):
    if request.user.is_authenticated:
        if request.user.role == ROLE[1][0]:
            activity_request = ActivityRequest.objects.filter(pk = request_id).select_related('category')
            if activity_request[0].category.owner == request.user:
                activity_request.update(status = STATUS[2][0])
